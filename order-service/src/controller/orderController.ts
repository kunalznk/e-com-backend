...
const createOrder = async (req: Request , res: Response) => {
    try {
        ...
        const options = {
            ...
            description: productsToBuy?.description,
            order_id: order.id,
            "notes": {
...
const getOrders = async (req: Request , res: Response) => {
    try {
        ...
        const { data , statusCode } = buildSuccessMessage(orders);
        res.status(statusCode).json(data);
    } catch (error) {
        const {data , statusCode } = buildFailMessage(error);
        res.status(statusCode).json(data);   
...
export default { createOrder , cancelOrder , returnOrder, getOrders, getOrdersById}