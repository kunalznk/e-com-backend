...
const addressLine2 = yup.string().optional().min(0).max(1000);
...
export const crateOrderSchema = yup.object({
  
  productId: yup.string().when('cartId', {
    is: false,
    then: yup.string().required("Please Select Product to buy"),
    otherwise: yup.string().optional()
  }),
  cartId: yup.string(),
  address: address.required("Please Enter valid address")
})
...
export const addToCartItemSchema = yup.object({
  productId: yup.string().required("product is required"),
  qty: yup.number().min(1).required("Qty is required"),
})